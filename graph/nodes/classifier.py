...
- "독감 예방접종은 언제 받는 것이 좋나요?"
    ...
    - follow_up = false:
      ...
      특히 이전 대화와 똑같은 단어나 텍스트가 뽑혀도 
      이전대화의 질의응답과 현재질문이 맥락상 이어지지 않으면 답변하지 마세요.

      (예를들어 배아파 질문 이전에 답변이 되었는데 그 다음질문에 배고파 있으면 '배'라는 텍스트가 있는데 배 라는 단어 뒤에 텍스트의 문맥이 다르므로 답변하지 마세요) 
      그리고 답변이 되지 않으면 참고문헌도 대답하지 마세요.
