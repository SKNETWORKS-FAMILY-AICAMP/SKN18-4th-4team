...
def filter_and_renumber_sources(answer: str, sources: list) -> tuple:
    ...
    for new_num, old_num in enumerate(sorted(used_numbers), start=1):
        if 1 <= old_num <= len(sources):
            ...
            # 문자열인 경우 딕셔너리로 변환하고 번호 추가
            if isinstance(source, str):
                filtered_sources.append({
                    "title": f"{new_num}. {source}"
                })
            else:
                # 이미 딕셔너리인 경우 번호 추가
                source_dict = dict(source) if isinstance(source, dict) else {"title": str(source)}
                original_title = source_dict.get("title", "")
                source_dict["title"] = f"{new_num}. {original_title}"
...
**답변 형식:**
[주요 내용]
핵심 개념이나 정의를 1-2문장으로 설명합니다. 반드시 출처 번호를 [1], [2] 형식으로 표시하세요.

[관련 상식 보충]
주요 내용과 관련된 추가 정보, 배경 지식, 또는 상세 설명을 제공합니다. 출처 번호를 포함하세요.
